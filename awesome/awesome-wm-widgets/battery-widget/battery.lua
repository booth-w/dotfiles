...
local function worker(user_args)
	...
	local warning_msg_text = args.warning_msg_text or 'Charge soon'
	...
	local function show_battery_status(batteryType)
		awful.spawn.easy_async([[bash -c 'acpi && echo -e "\nTotal time:" && python ~/scripts/battery.py']],
	...
	function(widget, stdout)
		...
		for i, batt in ipairs(battery_info) do
			if capacities[i] ~= nil then
								if status ~= 'Charging' and batt.status ~= 'Not charging' then
									status = batt.status
								end
		...
		if show_current_level then
			level_widget.text = string.format('%i%%', math.floor(charge));
		end
